...
wf = wave.open('micro.wav', 'w')  #open a wav file 
...
for n in range(0, N):
    ...
    # Difference equation
    y0 = b0*x0 + b2*x2 + b4*x4 - a1*y1 - a2*y2 - a3*y3 - a4*y4  #transfer function adding environmental noise
    ...
    # output_value = int(clip16(x0))   # Bypass filter (listen to input
    # directly)
    ...
    wf.writeframesraw(output_bytes) #save voice data into wave file
print('* Finished')
